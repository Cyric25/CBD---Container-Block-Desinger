...
| AP-1.fix3 | `showImageErrors` darf Export nicht abbrechen | sonnet | ☐ | AP-1.2, AP-1.rev | Auf Branch `phase-1-pdf-export-fixes`; behebt F2, noch ausstehend |
| AP-1.doc | Doku Phase 1 | sonnet | ☐ | AP-1.fix3 | |
...
| 2026-08-25 | AP-1.rev | Read-only Code-Review beider Branches (`git diff main...<branch>`), Stichproben gegen Akzeptanzkriterien, `php tools/check-php74.php` erneut ausgeführt | 4 von 6 Implementierungs-APs PASS, AP-1.4 FAIL (F1/F3), 16 Befunde dokumentiert (2 kritisch, 5 mittel, 9 gering); PHP-7.4-Check grün | Agent (unabhängig, keine Implementierung in dieser Phase) |
