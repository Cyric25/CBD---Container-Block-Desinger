...
| `test-inline-reference.php` | Vertrag D und E der Serverseite des Inline-Verweises (AP-3.3, nach TDD entstanden: roter Commit `d2e0597` vor grünem `0df80cd`; erweitert in AP-3.fix2). **155 Prüfungen** (119 + 36), **zweimal gefahren**: gegen die echte `WP_HTML_Tag_Processor` (aus einer gefundenen WordPress-Installation geborgt, ohne WordPress zu booten) und gegen ein schmales Doppel (`CBD_TEST_TAG_PROCESSOR=doppel`). Schwerpunkt ist die **Zeichengleichheit**: 23 Prüfungen belegen mit `===`, dass Inhalt ohne Inline-Verweis unangetastet bleibt — darunter `<script>` mit `if (a < b)`, eine JS-Regex, die vollständige `render.php`-Ausgabe des Blocks, Container-Markup mit `data-stable-id`, Umlaute und LaTeX. Dazu acht Fälle unbrauchbarer `data-target-post`, die href-Bildung mit und ohne Anker und mit vorhandener Abfrage, `get_permalink()` leer, Filterpriorität gegen die im Repo belegten 11 und 10000, und dass ohne `format.js` **nichts** registriert und **keine** Warnung ausgegeben wird. **Eine Prüfung fing einen Hoisting-Fehler im Harnisch selbst:** das unbedingt deklarierte Klassendoppel machte den Nachweis „fehlender Vertragspartner erzeugt keinen Fatal Error" wertlos — es steht jetzt in einem `eval()`. **Aus AP-3.fix2 dazugekommen:** überlange Ziffernfolgen (20, 30, 100 Stellen) müssen zeichengleich bleiben **und dürfen keine PHP-Warnung erzeugen** (eigener `set_error_handler` macht jede Warnung zum Fehlschlag), `PHP_INT_MAX` bleibt eine gültige ID, plus elf Struktur-Randfälle (Klasse an einem `<span>`, in `<script>`/`<style>`/`<textarea>`/HTML-Kommentar, als fremder Attributwert, in Großschreibung, unvollständiges Tag am Textende, zwei Klassen am Element, Anführungszeichen-Varianten, verschachtelte `<a>`). **Vier dieser Fälle kann das Doppel nicht tragen** (es kennt keine Rohtext-Elemente und keine Kommentare) und werden dort mit sichtbarer `SKIP`-Zeile samt Begründung übersprungen — ein still ausgelassener Test wäre schlimmer als ein fehlender |
| `test-block-auswahl.js` | Der gemeinsame Auswahlbaustein `window.cbdBlockAuswahl` (AP-3.2, nach TDD entstanden: roter Commit `5d34cf2` vor grünem `5fb4e8c`). **Läuft mit `node`, ohne jsdom und ohne npm-Abhängigkeit** — die Quelldatei wird über `new Function('window', quelle)` mit einem Stub-`window` ausgeführt. 133 Prüfungen: die sieben öffentlichen Namen und **kein achter**, Memoisierung (drei gleichzeitige Aufrufe = ein Promise, zwei `apiFetch`-Aufrufe), vier Netzfehler-Fälle ohne Reject, Kaskade mit gleichzeitiger Unterseiten- und Blockstufe, Beschneidung blockloser Zweige, Beiträge über die Wurzelmarke, `pfadVon()` mit unbekannter ID und mit Zyklus, Quellprüfung auf `import`/`export`/`class`/`console.log`. **`wp` wird als nicht aufzählbarer, protokollierender Getter gestubbt** — so ist beweisbar, dass die Datei es beim Laden nicht anfasst. **Nicht geprüft:** die Komponente `HierarchieAuswahl` selbst (ohne React-Umgebung nicht sinnvoll), nur ihr Wachposten; ihre Abnahme erfolgt an der Oberfläche |
| `test-seitenbaum.php` | Vertrag A und B der beiden Redakteurs-Routen (AP-3.1, nach TDD entstanden: roter Commit `85e1bc9` vor grünem `3a50704`; erweitert in AP-3.fix1, rot `1194e2b` vor grün `e933ee0`). **82 Prüfungen** (63 + 19): die drei neuen Felder in `cbd/v1/blocks`, Baumaufbau über vier Ebenen, Sortierung `menuOrder` vor `titel`, verwaister Knoten fällt samt Unterbaum heraus, Zyklus A→B→A von der Wurzel unerreichbar, Beiträge nicht im Baum, Memoisierung innerhalb einer Anfrage. **Zur `gesperrt`-Ermittlung (AP-3.fix1):** alle drei Stufen der Kette einzeln, mit Zählern auf **beiden** Theme-Funktionen — Stufe 1 genau einmal aufgerufen und Stufe 2 gar nicht, Abfragenzahl konstant bei 5 und bei 50 Seiten mit mindestens einer gesperrten Seite, und **eine Unterseite ohne eigene Meta erbt `gesperrt: true`**. Letzterer Fall fehlte in AP-3.1 vollständig — deshalb war dort die falsche Behauptung entstanden, die Theme-Funktion vererbe die Sperre nicht. **Quelltext-Zusicherung:** Prüfgruppe 3 liest die SQL-Zeichenkette selbst und verlangt fünf einzeln benannte Spalten, kein `SELECT *`, kein `post_content` |
